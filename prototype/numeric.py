...
class Rational:
    ...
    def __repr__ (self) -> str:
        ...
        if type(self.numerator) == int and type(self.denominator) == int and self.numerator in range(-9, 10) and self.denominator in range(1, 10):
...
class Term:
    ...
    def differentiate(self, var) -> Term:
        ...
        new_var_powers = self.var_powers.copy()
        coefficient = self.coefficient * self.var_powers[var]
        new_var_powers[var] -= 1

        return Term(coefficient, new_var_powers).simplify()
    
    # external function handles + C
    def anti_differentiate(self, var) -> Term:
        if var not in self.var_powers:
            new_var_powers = self.var_powers.copy()
            new_var_powers[var] = 1
            return Term(self.coefficient, new_var_powers).simplify()
        
        if self.coefficient != int:
            coefficient = self.coefficient/(self.var_powers[var]+1)
        else:
            coefficient = Rational(self.coefficient, self.var_powers[var]+1).simplify()
...
class Sum:
    ...
    def variables(self) -> list[str]:
        ...
        for term in self.term_list:
            if type(term) == Term:
                var_list += [var for var in term.var_powers if var not in var_list]
            if type(term) in (Sum, Product, LargePower):
                var_list += [var for var in term.variables if var not in var_list]

        return var_list
    
    def __iter__ (self):
        yield len(self.term_list)
...
class Product:
    ...
    def expand(self):
        # expand all in exp_list
        exp_list = []
        for exp in self.exp_list:
            if type(exp) in (Sum, LargePower):
    ...
    def variables(self) -> list[str]:
        ...
        for exp in self.exp_list:
            if type(exp) == Term:
                var_list += [var for var in exp.var_powers if var not in var_list]
            if type(exp) in (Sum, Product, LargePower):
                var_list += [var for var in exp.variables if var not in var_list]

        return var_list
    
    def __iter__ (self):
        yield len(self.exp_list)
        yield from self.exp_list
...
class LargePower:
    ...
    def variables(self) -> list[str]:
        var_list = []
        if type(self.base) == Term:
            var_list += [var for var in self.base.var_powers]
        elif type(self.base) in (Sum, Product, LargePower):
            var_list += [var for var in self.base.variables if var not in var_list]
        
        if type(self.power) == Term:
            var_list += [var for var in self.power.var_powers]
        elif type(self.power) in (Sum, Product, LargePower):
            var_list += [var for var in self.power.variables if var not in var_list]
